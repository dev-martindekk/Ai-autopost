        ...
        <div class="card mt-4">
            ...
            <div class="card-body">
                ...
                <?php if ($backupCodes): ?>
                    ...
                    <div class="row g-2 mb-3">
                        <?php foreach ($backupCodes as $code): ?>
                            <div class="col-6"><code class="d-block p-2 bg-light rounded text-center"><?= $code ?></code></div>
                        <?php endforeach; ?>
                    </div>
                    <div class="d-flex gap-2 mb-3">
                        <button type="button" class="btn btn-outline-primary btn-sm" onclick="copyBackupCodes()"><i class="fas fa-copy me-1"></i>คัดลอก</button>
                        <button type="button" class="btn btn-outline-secondary btn-sm" onclick="downloadBackupCodes()"><i class="fas fa-download me-1"></i>ดาวน์โหลด</button>
                    </div>
                    <script>
                    ...
                    function downloadBackupCodes() {
                        ...
                        a.href = URL.createObjectURL(new Blob(["AI AutoPost SEO - Backup Codes\nGenerated: <?= date('Y-m-d H:i:s') ?>\n\n" + backupCodes.join('\n')], {type:'text/plain'}));
                ...
                <?php if (!$isEnabled && !$setupData): ?>
                    <p class="text-muted small mb-3">เพิ่มความปลอดภัยด้วยรหัส 6 หลักจาก Google Authenticator ที่เปลี่ยนทุก 30 วินาที</p>
                    <form method="POST">
                        <input type="hidden" name="csrf_token" value="<?= $csrfToken ?>">
                        <input type="hidden" name="action" value="setup">
                        <button type="submit" class="btn btn-primary btn-sm"><i class="fas fa-shield-alt me-2"></i>เปิดใช้งาน 2FA</button>
                    </form>
                <?php endif; ?>

                <?php if ($setupData && !$isEnabled): ?>
                    <p class="fw-semibold small mb-2">1. สแกน QR Code ด้วย Google Authenticator</p>
                    <div class="text-center mb-3">
                        <div id="qrcode" class="d-inline-block border rounded p-2 bg-white"></div>
                        <p class="mt-1 mb-0 small text-muted">หรือกรอก: <code class="user-select-all"><?= htmlspecialchars($setupData['secret']) ?></code></p>
                    </div>
                    <script src="https://cdn.jsdelivr.net/npm/qrcodejs@1.0.0/qrcode.min.js"></script>
                    <script>new QRCode(document.getElementById("qrcode"),{text:<?= json_encode($setupData['otpauth_uri'] ?? $setupData['qr_url']) ?>,width:160,height:160,colorDark:"#000",colorLight:"#fff",correctLevel:QRCode.CorrectLevel.M});</script>
                    <p class="fw-semibold small mb-2">2. ป้อนรหัส 6 หลักเพื่อยืนยัน</p>
                    <form method="POST">
                        <input type="hidden" name="csrf_token" value="<?= $csrfToken ?>">
                        <input type="hidden" name="action" value="enable">
                        <input type="text" name="code" class="form-control text-center fw-bold mb-2"
                               placeholder="000000" maxlength="6" pattern="[0-9]{6}" required autofocus
                               style="letter-spacing:.3em;font-size:1.4rem;">
                        <button type="submit" class="btn btn-success w-100"><i class="fas fa-check me-2"></i>เปิดใช้งาน 2FA</button>
                    </form>
                <?php endif; ?>
                ...
                <?php if ($isEnabled): ?>
                    <div class="d-flex align-items-center gap-2 mb-3">
                        ...
                        <span class="fw-semibold text-success">2FA เปิดอยู่</span>
                        <small class="text-muted ms-1">· Backup codes เหลือ <?= $remainingCodes ?> รหัส</small>
                    </div>
                    <div class="row g-2">
                        <div class="col-6">
                            <form method="POST" onsubmit="return confirm('Backup Codes เดิมจะใช้ไม่ได้อีก ยืนยัน?')">
                                ...
                                <input type="password" name="password" class="form-control form-control-sm mb-1" placeholder="รหัสผ่าน" required>
                                <button type="submit" class="btn btn-warning btn-sm w-100"><i class="fas fa-sync me-1"></i>สร้าง Backup Codes ใหม่</button>
                            </form>
                        </div>
                        <div class="col-6">
                            <form method="POST" onsubmit="return confirm('ปิด 2FA จะลดความปลอดภัยของบัญชี ยืนยัน?')">
                                ...
                                <input type="password" name="password" class="form-control form-control-sm mb-1" placeholder="รหัสผ่าน" required>
                                <button type="submit" class="btn btn-outline-danger btn-sm w-100"><i class="fas fa-lock-open me-1"></i>ปิด 2FA</button>
                            </form>
                        </div>
                    </div>
                <?php endif; ?>
                ...
                <?php else: ?>
                    <div class="table-responsive">
                        <table class="table table-hover table-sm mb-0">
                            <thead><tr><th>เวลา</th><th>กิจกรรม</th><th>Level</th></tr></thead>
                            <tbody>
                                <?php foreach ($activities as $a): ?>
                                    <tr>
                                        <td><small><?= date('d/m/Y H:i', strtotime($a['created_at'])) ?></small></td>
                                        <td><small><?= sanitize($a['message']) ?></small></td>
                                        <td><?php $lc = match($a['log_type'] ?? 'info') {
                                            'error' => 'danger', 'warning' => 'warning', 'info' => 'info', default => 'secondary'
                                        }; ?><span class="badge bg-<?= $lc ?>"><?= strtoupper($a['log_type'] ?? 'info') ?></span></td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
